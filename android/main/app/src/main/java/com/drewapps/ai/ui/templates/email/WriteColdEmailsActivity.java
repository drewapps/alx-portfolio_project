package com.drewapps.ai.ui.templates.email;
...
import com.drewapps.ai.databinding.ActivityWriteColdEmailsBinding;
...
public class WriteColdEmailsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding = ActivityWriteColdEmailsBinding.inflate(getLayoutInflater());
    ...
    private void initUI() {
        ...
        binding.tiLanguage.setEndIconOnClickListener(v -> {
            Tools.ShowPopUpMenu(WriteColdEmailsActivity.this, v, binding.etLanguage,
        ...
        binding.tilOutput.setEndIconOnClickListener(v -> {
            Tools.ShowPopUpMenu(WriteColdEmailsActivity.this, v, binding.etOutput,
        ...
        binding.btnGenerate.setOnClickListener(v -> {
            ...
            binding.tilSenderName.setErrorEnabled(false);
            binding.tilRecipientAbout.setErrorEnabled(false);
            binding.tilSenderInfo.setErrorEnabled(false);
            if (validate()) {
        ...
        binding.btnClear.setOnClickListener(v -> {
            ...
            binding.etSenderName.setText("");
            binding.etRecipientAbout.setText("");
            binding.etSenderInfo.setText("");
        });

        binding.etDocumentName.setOnClickListener(v -> {
            binding.tilDocumentName.setErrorEnabled(false);
        });
        binding.etSenderName.setOnClickListener(v -> {
            binding.tilSenderName.setErrorEnabled(false);
        });
        binding.etRecipientAbout.setOnClickListener(v -> {
            binding.tilRecipientAbout.setErrorEnabled(false);
        });
        binding.etSenderInfo.setOnClickListener(v -> {
            binding.tilSenderInfo.setErrorEnabled(false);
        });
    }
    ...
    private void startGenerating() {
        ...
        String senderName = binding.etSenderName.getText().toString();
        String recName = binding.etRecipientAbout.getText().toString();
        String senderInfo = binding.etSenderInfo.getText().toString();
        ...
        executor.execute(() -> {
            ...
            try {
                ...
                Response<Document> response = ApiClient.getApiService().write_cold_email(Config.API_KEY,
                        documentName, template.templateId, MyApplication.prefManager().getInt(Constants.USER_ID),
                        senderName, recName, senderInfo, output, language, emoji).execute();
                ...
                if (apiResponse.isSuccessful()) {
                    ...
                    runOnUiThread(() -> {
                        ...
                        dialogMsg.cancel();
                        Intent intent = new Intent(WriteColdEmailsActivity.this, EditorActivity.class);
    ...
    private boolean validate() {
        if (binding.etDocumentName.getText().toString().isEmpty()) {
            ...
            return false;
        } else if (binding.etSenderName.getText().toString().isEmpty()) {
            binding.tilSenderName.setError("Sender's Name is required");
            binding.tilSenderName.setErrorEnabled(true);
            binding.etSenderName.requestFocus();
            return false;
        } else if (binding.etRecipientAbout.getText().toString().isEmpty()) {
            binding.tilRecipientAbout.setError("Recipient's Name is required");
            binding.tilRecipientAbout.setErrorEnabled(true);
            binding.etRecipientAbout.requestFocus();
            return false;
        } else if (binding.etSenderInfo.getText().toString().isEmpty()) {
            binding.tilSenderInfo.setError("Sender's Information is required");
            binding.tilSenderInfo.setErrorEnabled(true);
            binding.etSenderInfo.requestFocus();
            return false;
        }

...
import com.drewapps.ai.databinding.ActivitySynonymGeneratorBinding;
...
public class SynonymGeneratorActivity extends AppCompatActivity {

    ActivitySynonymGeneratorBinding binding;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding = ActivitySynonymGeneratorBinding.inflate(getLayoutInflater());
    ...
    private void initUI() {
        ...
        if(template.templateName.equals("Antonyms Generator")){
            isAntonym = true;
            binding.tilOutput.setHint("No. of Antonyms");
        }
        ...
        binding.etOutput.setOnClickListener(v -> {
            Tools.ShowPopUpMenu(SynonymGeneratorActivity.this, v, binding.etOutput,
    ...
    private void startGenerating() {
        ...
        String word = binding.etQuestion.getText().toString();
        ...
        executor.execute(() -> {
            ...
            try {
                ...
                if(isAntonym){
                    response = ApiClient.getApiService().antonym_generator(Config.API_KEY,
                            documentName, template.templateId, MyApplication.prefManager().getInt(Constants.USER_ID),
                            word, output).execute();
                }else {
                    response = ApiClient.getApiService().synonym_generator(Config.API_KEY,
                            documentName, template.templateId, MyApplication.prefManager().getInt(Constants.USER_ID),
                            word, output).execute();
                }
                ...
                if (apiResponse.isSuccessful()) {
                    ...
                    runOnUiThread(() -> {
                        ...
                        dialogMsg.cancel();
                        Intent intent = new Intent(SynonymGeneratorActivity.this, EditorActivity.class);
    ...
    private boolean validate() {
        if (binding.etDocumentName.getText().toString().isEmpty()) {
            ...
            binding.tilQuestion.setError("Word is required");

...
import com.drewapps.ai.databinding.ActivityTwitterTweetsBinding;
...
public class TwitterTweetsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding = ActivityTwitterTweetsBinding.inflate(getLayoutInflater());
    ...
    private void initUI() {
        ...
        if (template.templateName.equals("Pinterest Board Ideas") || template.templateName.equals("Pinterest Pin ideas")) {
            binding.swEmoji.setVisibility(View.GONE);
        }

        if (template.templateName.equals("Board Descriptions")) {
            binding.swEmoji.setVisibility(View.GONE);
            binding.tilInstruction.setHint("Title");
            binding.tilInstruction.setHelperText("Title*");
        }
        if (template.templateName.equals("Pin Group Ideas") || template.templateName.equals("Snapchat Content Ideas")
                || template.templateName.equals("Snapchat Spotlight Ideas")) {
            binding.swEmoji.setVisibility(View.GONE);
            binding.tilInstruction.setHint("Theme");
            binding.tilInstruction.setHelperText("Theme*");
        }

        if (template.templateName.equals("Snapchat Story Series")) {
            binding.swEmoji.setVisibility(View.GONE);
        ...
        binding.etOutput.setOnClickListener(v -> {
            Tools.ShowPopUpMenu(TwitterTweetsActivity.this, v, binding.etOutput,
        ...
        binding.etLanguage.setOnClickListener(v -> {
            Tools.ShowPopUpMenu(TwitterTweetsActivity.this, v, binding.etLanguage,
    ...
    private void startGenerating() {
        ...
        executor.execute(() -> {
            ...
            try {
                ...
                if (template.templateName.equals("Pinterest Board Ideas") || template.templateName.equals("Pinterest Pin ideas")) {
                    response = ApiClient.getApiService().pin_board_idea(Config.API_KEY,
                            documentName, template.templateId, MyApplication.prefManager().getInt(Constants.USER_ID),
                            instruction, output, language).execute();
                } else if (template.templateName.equals("Board Descriptions")) {
                    response = ApiClient.getApiService().board_description(Config.API_KEY,
                            documentName, template.templateId, MyApplication.prefManager().getInt(Constants.USER_ID),
                            instruction, output, language).execute();
                } else if (template.templateName.equals("Snapchat Story Series")) {
                    response = ApiClient.getApiService().snapchat_series(Config.API_KEY,
                            documentName, template.templateId, MyApplication.prefManager().getInt(Constants.USER_ID),
                            instruction, language).execute();
                } else if (template.templateName.equals("Pin Group Ideas") || template.templateName.equals("Snapchat Content Ideas")
                        || template.templateName.equals("Snapchat Spotlight Ideas")) {
                    response = ApiClient.getApiService().pin_group_idea(Config.API_KEY,
                            documentName, template.templateId, MyApplication.prefManager().getInt(Constants.USER_ID),
                            instruction, output, language).execute();
                } else {
                    response = ApiClient.getApiService().twitter_tweets(Config.API_KEY,
                            documentName, template.templateId, MyApplication.prefManager().getInt(Constants.USER_ID),
                            instruction, output, emoji, language).execute();
                }
                ...
                if (apiResponse.isSuccessful()) {
                    ...
                    runOnUiThread(() -> {
                        ...
                        dialogMsg.cancel();
                        Intent intent = new Intent(TwitterTweetsActivity.this, EditorActivity.class);
    ...
    private boolean validate() {
        if (binding.etDocumentName.getText().toString().isEmpty()) {
            ...
            binding.tilInstruction.setError(template.templateName.equals("Board Descriptions") ? "Title is Required" :
                    template.templateName.equals("Pin Group Ideas") ? "Theme is Required" :
                            template.templateName.equals("Snapchat Content Ideas") ? "Theme is Required" :
                                    template.templateName.equals("Snapchat Spotlight Ideas") ? "Theme is Required" :
                                            template.templateName.equals("Snapchat Story Series") ? "Theme is Required" : "Topic is required");

  ...
  <div class="col-md-12">
    ...
    <div class="card card-primary">
        <div class="card-header">
            ...
            <a href="{{ route('plan.create')}}" class="btn btn-success float-right">Add New</a>
        </div> 
        ...
        <table class="table" id="data_table">
          ...
          @foreach($data as $row)
            <tr>
              ...
              <td class="align-middle"><b>{{$row->plan_name}}</b></td>
              <td class="align-middle">{{$row->plan_price}} USD</td>
              <td class="align-middle">{{$row->total_words}}</td>
              <td class="align-middle">{{$row->number_of_images}}</td>
              ...
              <td class="align-middle">
                <div class="btn-group">
                  <a href="{{url('admin/plan/'.$row->id.'/edit') }}"><button type="button" class="btn btn-success"><span aria-hidden="true" class="fa fa-edit"></span></button></a>
                ...
                {!! Form::open(['url' => 'admin/plan/'.$row->id,'method'=>'DELETE','class'=>'form-horizontal','id'=>'form_'.$row->id]) !!}
...
<script type="text/javascript">
    ...
    $(".most_popular").change(function(){
      ...
      $.ajax({
        type: "POST",
        url: "{{url('admin/plan-most-popular')}}",
        data: { checked : checked , id : id},
        headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
        success: function(data) {
          location.reload();
